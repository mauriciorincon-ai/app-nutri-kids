...
| 2026-07-07 | Fase 4 completa: 7 e2e del chat (móvil+desktop, provider `mock`): lookup con 0 requests al LLM, sugerencia, stream grounded, rechazo médico sin insignia IA, kill-switch degradado, no-persistencia al recargar, EN. axe extendido a `/chat`. 3 ADRs (004 proveedor+Groq verificado, 005 grounding tokens medidos, 006 privacidad). Manual += "Pregúntale a la dieta". Lighthouse `/chat`: LCP 3766 < 4200, script 320 < 380 KB, todas las métricas dentro del budget. Gate 6+1 verde en local (typecheck/lint/118 unit/build/audit-high/44 e2e). Barrido de privacidad limpio (solo demo, sin secrets). Estándar 7 con adaptación declarada (salida efímera, sin persistencia → ADR 006). Summary generado                                                                                                                    |
