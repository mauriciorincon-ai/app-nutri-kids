...
| 2026-07-07 | **Sentry verificado en la preview de Vercel:** el usuario creó cuenta (GitHub SSO) + proyecto, configuró `NEXT_PUBLIC_SENTRY_DSN` en Vercel (Production+Preview); tras redeploy fresco, la preview dispara `envelope/?sentry_version=...` con **200** (DevTools del usuario). Primer intento fallaba por build congelada previa a la env var. Herramienta dev-only `scripts/check-sentry.mjs` para repetir la verificación                                                                                                                                                                                                                                                                     |
